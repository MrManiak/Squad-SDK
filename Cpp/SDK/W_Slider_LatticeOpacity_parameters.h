#pragma once

// Name: Sq, Version: b21

#include "../SDK.h"

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

/*!!HELPER_DEF!!*/

/*!!DEFINE!!*/

namespace UFT
{
//---------------------------------------------------------------------------
// Parameters
//---------------------------------------------------------------------------

// Function W_Slider_LatticeOpacity.W_Slider_LatticeOpacity_C.BndEvt__SettingsItem_Slider_K2Node_ComponentBoundEvent_6_OnCaptureEnd__DelegateSignature
struct UW_Slider_LatticeOpacity_C_BndEvt__SettingsItem_Slider_K2Node_ComponentBoundEvent_6_OnCaptureEnd__DelegateSignature_Params
{
	float                                              Value;                                                     // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};

// Function W_Slider_LatticeOpacity.W_Slider_LatticeOpacity_C.Set Value
struct UW_Slider_LatticeOpacity_C_Set_Value_Params
{
	float                                              Value;                                                     // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};

// Function W_Slider_LatticeOpacity.W_Slider_LatticeOpacity_C.Construct
struct UW_Slider_LatticeOpacity_C_Construct_Params
{
};

// Function W_Slider_LatticeOpacity.W_Slider_LatticeOpacity_C.ExecuteUbergraph_W_Slider_LatticeOpacity
struct UW_Slider_LatticeOpacity_C_ExecuteUbergraph_W_Slider_LatticeOpacity_Params
{
	int                                                EntryPoint;                                                // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
